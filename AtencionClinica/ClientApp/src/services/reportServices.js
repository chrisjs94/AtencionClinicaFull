const urlReport = () => 
{
    const urlPre = "https://localhost:44386/";
    const urlPro = "http://192.168.1.251/reportes/";

    const urlBase = process.env.NODE_ENV == "development" ? urlPre : urlPro;
    ...
    return {
        ...
        testsResultTicket: id => `${urlBase}testservices?id=${id}&sizePage=TICKET`,
        privateTestsResult: id => `${urlBase}/testservices/privateTest?id=${id}`,
        privateTestsResultTicket: id => `${urlBase}/testservices/privateTest?id=${id}&sizePage=TICKET`,
